<template>
    <div>
        <NavigationItems v-bind="this.$attrs" @openMenu="push" @closeMenu="pull">
            <slot></slot>
        </NavigationItems>
    </div>
</template>
...
<script>
    ...
    export default {
      ...
      methods: {
        ...
        push() {
          this.openMenu()
          let width = this.$attrs.width ? this.$attrs.width + 'rem' : '10rem';
          console.log(width);
          ...
          if (this.$attrs.right) {
            ...
            document.querySelector(
              '#page-wrap'
            ).style.transform = `translate3d(${width}, 0rem, -30rem ) `;
          }
